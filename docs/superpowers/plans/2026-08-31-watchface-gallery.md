...
`build_factory_seed.py` builds the staging tree and invokes the ESP-IDF LittleFS partition image target. `flash_factory.py` requires the repository-pinned `esptool==5.3.1`, validates explicit port syntax and its imported version, and performs parser-only validation of complete chip-id/erase/write/no-stub-chip-id-hard-reset vectors without invoking callbacks or opening a device. It then confirms the requested port appears exactly once in read-only PlatformIO USB serial discovery. The Make factory-flash target completes that tooling/port preflight before building packages or the seed, and invokes only the selected Python module rather than an ambient executable. It builds and hashes the local images, verifies the exact CSV and binary partition layout plus classic-ESP32 target, erases only NVS at `0x9000`/`0x6000` with the post-operation reset disabled, then writes bootloader, partition table, firmware, and the exact `0x230000`-byte LittleFS image at `0x1d0000`, also without resetting. Only after that complete write reports success does a separate `--before no-reset --after hard-reset --no-stub chip-id` identity read reset the board into the application via RTS without mutating flash or NVS, so the retained application cannot run between mutations or after a partial write. The earlier `run` handoff is intentionally prohibited because esptool 5.3.1 attaches SPI flash before running and failed on physical ESP32-PICO-D4 hardware. This intentionally clears settings, Wi-Fi credentials, package index/health state, and the seed marker so every factory reinstall imports a coherent gallery. CI and the documented local gallery gate install both pinned `tools/font-requirements.txt` and `tools/factory-flash-requirements.txt` into the exact Python interpreter used for tests. The Make gate uses a distinct `IDF_PYTHON` (default `python3`) for builds that require the installed ESP-IDF Python environment, while `PYTHON` remains the small pinned test/factory-tool environment. Normal `platformio run -e watchy_v2 -t upload` remains firmware-only.
