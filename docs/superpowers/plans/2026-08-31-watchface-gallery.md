...
`build_factory_seed.py` builds the staging tree and invokes the ESP-IDF LittleFS partition image target. `flash_factory.py` requires the repository-pinned `esptool==5.3.1`, validates its imported version and every used parser form before any build/device runner, and invokes only that Python module rather than an ambient executable. It builds and hashes the local images, verifies the exact CSV and binary partition layout plus classic-ESP32 target, erases only NVS at `0x9000`/`0x6000` with the post-operation reset disabled, then writes bootloader, partition table, firmware, and the exact `0x230000`-byte LittleFS image at `0x1d0000`, also without resetting. Only after that complete write reports success does a separate non-mutating ROM-loader `run` with `--no-stub` start the application, so the retained application cannot run between mutations or after a partial write. This intentionally clears settings, Wi-Fi credentials, package index/health state, and the seed marker so every factory reinstall imports a coherent gallery. Normal `platformio run -e watchy_v2 -t upload` remains firmware-only.
