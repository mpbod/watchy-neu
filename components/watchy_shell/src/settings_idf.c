...
static void get_string(nvs_handle_t handle, const char *key, char *value, size_t capacity) {
    size_t size = capacity;
    char temporary[WATCHY_PACKAGE_REF_MAX + 1u];
    if (capacity > sizeof(temporary) ||
        nvs_get_str(handle, key, temporary, &size) != ESP_OK || size == 0u || size > capacity) {
        return;
    }
...
watchy_status_t watchy_settings_load(watchy_settings_t *out_settings) {
    ...
    get_string(handle, "tz", stored.timezone, sizeof(stored.timezone));
    if (nvs_get_u8(handle, "hour24", &value) == ESP_OK && value <= 1u) {
        stored.time_24h = value == 1u;
    }
    if (nvs_get_u8(handle, "motion", &value) == ESP_OK && value <= 1u) {
        stored.motion_wake = value == 1u;
    }
    get_string(handle, "face", stored.active_watchface, sizeof(stored.active_watchface));
    get_string(handle, "ssid", stored.wifi_ssid, sizeof(stored.wifi_ssid));
    get_string(handle, "wifi_pass", stored.wifi_password, sizeof(stored.wifi_password));
    if (nvs_get_u16(handle, "partial", &partial) == ESP_OK) {
        stored.partial_refresh_limit = partial;
    }
    get_string(handle, "ntp", stored.ntp_server, sizeof(stored.ntp_server));
    nvs_close(handle);
